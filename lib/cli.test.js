...
describe('initArgs + parseArgs', () => {
    ...
    test('--help forces exit', () => {
        ...
        (0, cli_1.initArgs)(['https://www.google.com', '--help']);
    ...
    test('--version forces exit', () => {
        // Mock console.log to not pollute the log with the yargs help text
        const mockLog = jest.spyOn(console, 'log').mockImplementation();
        (0, cli_1.initArgs)(['https://www.google.com', '--version']);
        expect(mockExit).toHaveBeenCalledTimes(1);
        expect(mockLog).toBeCalled();
        mockLog.mockRestore();
    });
    ...
    test('first positional becomes targetUrl', () => {
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com']));
        expect(args.targetUrl).toBe('https://google.com');
        expect(args.upgrade).toBeUndefined();
    });
    test('second positional becomes out', () => {
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', 'tmp']));
    ...
    test('upgrade arg', () => {
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['--upgrade', 'pathToUpgrade']));
        expect(args.upgrade).toBe('pathToUpgrade');
        expect(args.targetUrl).toBeUndefined();
    });
    test('upgrade arg with out dir', () => {
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['tmp', '--upgrade', 'pathToUpgrade']));
        expect(args.upgrade).toBe('pathToUpgrade');
        expect(args.out).toBe('tmp');
        expect(args.targetUrl).toBeUndefined();
    });
    test('upgrade arg with targetUrl', () => {
        expect(() => (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://www.google.com', '--upgrade', 'path/to/upgrade']))).toThrow();
    });
    test('multi-inject', () => {
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)([
            'https://google.com',
            '--inject',
            'test.js',
            '--inject',
    ...
    test.each([
        ...
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `--${arg}`, value]));
        if (!isJsonString) {
            expect(args[arg]).toBe(value);
        }
        else {
            expect(args[arg]).toEqual((0, parseUtils_1.parseJson)(value));
        }
        if (shortArg) {
            const argsShort = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `-${shortArg}`, value]));
            if (!isJsonString) {
                expect(argsShort[arg]).toBe(value);
            }
            else {
                expect(argsShort[arg]).toEqual((0, parseUtils_1.parseJson)(value));
            }
        }
    });
    test.each([
        ...
        (0, cli_1.initArgs)(['https://google.com', `--${arg}`, badValue]);
        expect(mockExit).toHaveBeenCalledTimes(1);
        ...
        if (shortArg) {
            const argsShort = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `-${shortArg}`, value]));
            expect(argsShort[arg]).toBe(value);
            (0, cli_1.initArgs)(['https://google.com', `-${shortArg}`, badValue]);
            expect(mockExit).toHaveBeenCalledTimes(1);
            expect(mockError).toBeCalled();
        }
    ...
    test.each([
        ...
        const defaultArgs = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com']));
        expect(defaultArgs[arg]).toBe(false);
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `--${arg}`]));
        expect(args[arg]).toBe(true);
        if (shortArg) {
            const argsShort = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `-${shortArg}`]));
            expect(argsShort[arg]).toBe(true);
        }
    });
    test.each([{ arg: 'no-overwrite', shortArg: '' }])('test inversible boolean arg %s', ({ arg, shortArg }) => {
    ...
    test.each([
        ...
        const args = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `--${arg}`, `${value}`]));
        expect(args[arg]).toBe(value);
        const badArgs = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `--${arg}`, 'abcd']));
        expect(badArgs[arg]).toBeNaN();
        if (shortArg) {
            const shortArgs = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `-${shortArg}`, `${value}`]));
            expect(shortArgs[arg]).toBe(value);
            const badShortArgs = (0, cli_1.parseArgs)((0, cli_1.initArgs)(['https://google.com', `-${shortArg}`, 'abcd']));
